...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    addPoint: (state, newPoint) => state.map.points.push(newPoint),
    removePoint: (state, oldPoint) =>
      (state.map.points = state.map.points.filter(
  ...
  actions: {
    ...
    newMap: context => {
      ...
      db.collection("maps")
        .add(map)
        .then(doc =>
          context.dispatch("editMap", Object.assign(map, { id: doc.id }))
        );
    },
    newTag: () => {
      var tag = {
    ...
    search(context, event) {
      console.log(event);
      if (context.state.search.length >= 3) console.log("SIKERLER");
    }
  }
});
